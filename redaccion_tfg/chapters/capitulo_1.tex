...
Los únicos puntos de equilibrio posibles son: $(S^*,I^*)=(0,N)$ y $(S^*,I^*)=(N,0)$, y como sabemos que tenemos condiciones iniciales positivas y es claro que $S_n$ es monótonamente decreciente e $I_n$ es monótonamente creciente (ya que $S_{n+1}$ es $S_n$ multiplicado por un valor menor que $1$, mientras que $I_{n+1}$ corresponde a $I_n$ multiplicado por un valor mayor que $1$), así $S_{n+1}<S_n$ y $I_{n+1}>I_n$ para cualquier $n\in\mathbb{N}$, entonces debe converger a $(S^*,I^*)=(0,N)$, pues son sucesiones monótonas acotadas.
...
En la figura \ref{fig: SI_IsobreS}, que representa el número de individuos infectados en función del de susceptibles se aprecia una relación lineal per inversamente proporcional, donde al subir el número de infectados baja el de susceptibles y al contrario.
...
Por hipótesis inicial, tenemos que $S_0, I_0, R_0>0$ y $S_0+I_0+R_0=N$, luego su suficiente comprobar que:
...
Para probar el recíproco basta probar que ambas cantidades son menores o iguales a $1$. Comenzamos trabajando en la primera ecuación \eqref{eqn: SIR_modelo}, suponemos la positividad para $n$ y probándola para $n+1$:
$$S_{n+1}=S_n\left(1-\frac{\alpha\Delta t}{N}I_n\right)$$
...
Por tanto, el intervalo de tiempo debe ser menor que el tiempo medio requerido para contagiar a oro individuo susceptible y menor que el período medio infeccioso.
...
Cabe destacar que para cada valor de los infectados hay dos valores de recuperados. Otra cosa a tener en cuenta es la distancia entre las marcas en la curva. Cada marca indica la medida del número de infectados y recuperados en un mismo tiempo $t$.
...
En el modelo SIS la restricción necesaria para garantizar soluciones positivas no es suficiente para asegurar la convergencia. En este caso a dicha restricción hay que añadir la condición $\alpha \Delta t \leq 2+\gamma \Delta t$.
...
Cabe destacar el parecido de esta gráfica con su análoga del modelo SI (\ref{fig: SI_IsobreS}.
...
Al comienzo de la pandemia podemos asumir que $S(t)$ y $\tau (t)$ son constantes e iguales a $S_0$ y $\tau_0 = \tau (t_0)$ respectivamente. Sustituyendo estos valores en la ecuación \eqref{eqn: SI_cont} obtenemos:
