...
Se han obtenido los datos de \cite{datos_vih}.

El VIH es una enfermedad sin cura, por lo que los individuos infectados nunca dejan de serlo. Por ello, esta enfermedad sigue un modelo SI.
...
Vamos a ajustar solo el principio de la pandemia, en concreto vamos a ajustar a partir del día de 11 de Febrero de 2020, ya que en los 10 días anteriores no hay datos reportados pese a que aparecen en los datos, y durante los sigueintes 170 días, esto es, unos 6 meses, pues es aproximadamente 6 meses, lo que se estima duró la primera ola.
...
En \eqref{estimacion_covid} se muestra la estimación de los parámetros obtenidos para el ajuste, resultando un $\alpha \approx 0.0.21$, $\gamma \approx 0.08$, $I_0 \approx 1$, $R_0 \approx 536$. Dado que los valores de estos parámetros son razonables, salvo quizá por una estimación ligeramente alta de los recuperados iniciales, y la representación gráfica del ajuste aproxima relativamente bien los datos obtenidos, parece que las infecciones por covid en España en efecto siguen un modelo SIR.
