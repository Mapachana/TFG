...
\textcolor{red}{Incluir referencia}
...
Para este modelo, se ha usado un cambio de notación respecto a los modelos que hemos estudiado antes. En este caso, a la tasa de contacto, antes llamada $\alpha$, se ha denotado como $\beta$, y a la probabilidad de que un infectado pase a recuperado, anteriormente denominada $\gamma$, se la ha denotado $\alpha$.
...
\textcolor{red}{Añadir esquema del libro}

De acuerdo con la primera suposición, dado que la probabilidad de que aleatoriamente el contacto de un infectado sea con un individuo susceptible es $\frac{S}{N}$, el número de nuevos infectados por unidad de tiempo por infectado es $\frac{\beta NS}{N}$, dando lugar a una tasa de nuevos infectados de $\frac{\beta  N S I}{N}=\beta SI$. Notemos que ambas aproximaciones dan lugar a la misma tasa de nuevos infectados.
...
Por tanto, la fracción de infectados que siguen estando en la clase infectados $s$ unidades de tiempo después de haberse convertido en infectados es $e^{-\alpha s}$, luego la duración del período infeccioso se distribuye según una exponencial, con significado
$$\int_0^\infty e^{-\alpha s}ds=1/\alpha,$$
...
Es decir, $I$ aumenta mientras se cumpla que $S>\frac{\alpha}{\beta}$, pero como $S$ decrece para todo $t$, finalmente se tiene que $S<\frac{\alpha}{\beta}$, y por tanto $I$ deja de aumentar.

Si $S_0<\frac{\alpha}{\beta}$, entonces $I$ decrece a $0$ y no hay una epidemia, mientras que si $S_0>\frac{\alpha}{\beta}$, $I$ comienza creciendo hasta el máximo obtenido cuando $S=\frac{\alpha}{\beta}$ y luego decrece hasta $0$, en cuyo caso hay una epidemia.
